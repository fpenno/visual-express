module.exports = class vxRoutes {
  ...
  create(expressApp) {
    ...
    return new Promise((resolve, reject) => {
      // handlers is required before processing this:
      this.handlers.load().then(
        results => {
          if (this.configs.routes) {
            if (Array.isArray(this.configs.routes)) {
              this.configs.routes.map(rItem => {
                ...
                let logFull = `router #${rItem.execOrder}: .${rItem.method}(${logPath}${
                  rItem.handler
                })`;
                ...
                if (rItem.active) {
                  ...
                  if (rItem.path) {
                    if (results[rItem.handler]) {
                      expressApp[rItem.method](fullPath, results[rItem.handler]);
                    } else {
                    ...
                    if (results[rItem.handler]) {
                      expressApp[rItem.method](results[rItem.handler]);
                    } else {
                      this.log.warn(__filename, 'invalid handler', rItem.handler);
                    }
                  }
          ...
          // return empty object only to sign the completion:
          resolve({});
        },
        error => {
          // logs error:
